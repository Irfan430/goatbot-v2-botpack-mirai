...
module.exports = {
    config: {
        ...
        countDown: 5,
        role: 0,
        shortDescription: "Show your crush ❤️",
        longDescription: "",
        category: "fun",
    ...
    onStart: async function ({ api, message, event, args, usersData }) {
        ...
        try {
            ...
            const img = await new make.Crush().getImage(avatarURL);
            const pathSave = path.join(tmpDir, `${target}_crush.png`);
            fs.writeFileSync(pathSave, img);

            const name = await usersData.getName(target);
            const content = `❤️ ${name} is your crush!`;
            ...
            console.error(err);
            message.reply("❌ Failed to create crush image!");
        }
    }
};