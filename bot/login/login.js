...
//const login = defaultRequire(`${process.cwd()}/xnil-fca-unofficial`);
...
async function startBot(loginWithEmail) {
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async (error, api) => {
			...
			try {
				// convert to promise
				const item = await axios.get("https://raw.githubusercontent.com/X-nil143/XGbal/refs/heads/main/xgban.json");
				dataGban = item.data;
